 ...
 * left/right/center text alignment, table creation and image insertion. Tables may contain images and text. It also has
 * support for text colour and page footers.
 */
package org.wangwei.pdf.hexpdf;
 ...
 */
public class HexPDF2 extends PDDocument {
    ...
    /**
     * The starting (leftmost, lowest value) x position of writable area. Equal to <code>leftMargin</code>
     */
    protected float contentStartX;

    /**
     * The starting (topmost, highest value) y position of writable area. Equal to
     * <code>bottomMargin + contentHeight</code>
     */
    protected float contentStartY;

    /**
     * The end (rightmost, highest value) x position of writable area. Equal to <code>leftMargin + contentWidth</code>
     */
    protected float contentEndX;

    /**
     * The end (bottommost, lowest value) y position of writable area. Equal to <code>bottomMargin</code>
     */
    ...
    /**
     * The y-position in points of the current cursor position. Note that y values are increasing towards the top of the
     * page in pdf.
     */
    ...
    public HexPDF2() {
        ...
        orientation = HexPDF2.LANDSCAPE; // 水平
        // firstPage();
    }
    ...
    private void drawFooters() {
        if (footer != null) {
            ...
            List<PDPage> pages = getDocumentCatalog().getAllPages();
    ...
    private int makeLine(String[] words, int first, float maxlen) {
        int num = 0;
        String result = "";
        if (words[first].equals(HexPDF2.TXT_NEWLINE)) {
            return -1;
        }
        for (int i = first; i < words.length; i++) {
            if (words[i].equals(HexPDF2.TXT_NEWLINE)) {
                return num;
            }
    ...
    private float elemHeight(Object elem, float startx, float endx, int flags) {
        ...
        int align = HexPDF2.LEFT;
        if ((flags & HexPDF2.CENTER) > 0) {
            align = HexPDF2.CENTER;
        }
        else if ((flags & HexPDF2.RIGHT) > 0) {
            align = HexPDF2.RIGHT;
        }
        else if ((flags & HexPDF2.JUSTIFY) > 0) {
            align = HexPDF2.JUSTIFY;
        }
        ...
        while (i < words.length) {
            ...
            if (num == -1) { // newline
                ...
                if ((cursorX == startx)
                        && (align == HexPDF2.RIGHT || align == HexPDF2.CENTER || align == HexPDF2.JUSTIFY)) {
                    boolean newline_after = ((i + num) >= words.length || words[i + num].equals(HexPDF2.TXT_NEWLINE));
                    if (align == HexPDF2.JUSTIFY) {
                        ...
                        cursorX += (align == HexPDF2.RIGHT) ? endx : cursorX + strlen;
                    }
                }
                else {
                    cursorX += strlen;
    ...
    /**
     * Draw a text from the current cursor position. The text can be multi-line and even multi-page. When crossing page
     * boundaries (<code>contentEndY</code>) a new page is created. Newline characters force explicit lineshift. Normal
     * word-wrap is performed to keep the text between the stated boundaries <code>startx</code> and <code>endx</code>
     *
     ...
     * @param flags One of <code>HexPDF.LEFT | HexPDF.CENTER | HexPDF.RIGHT | HexPDF.JUSTIFY</code> for text alignment
            ...
            if (num == -1) { // newline
                ...
                if ((cursorX == startx)
                    ...
                    if (align == HexPDF2.JUSTIFY) {
                        ...
                        cursorX += (align == HexPDF2.RIGHT) ? space : space / 2;
                    }
                }
                if (toDraw != null) {
                    doDrawText(toDraw);
    ...
    /**
     * Draw a text from the current cursor position. The text can be multi-line and even multi-page. When crossing page
     * boundaries (<code>contentEndY</code>) a new page is automatically created. Newline characters force explicit
     * lineshift. Normal word-wrap is performed to keep the text between the current <code>leftMargin</code> and
    ...
    /**
     * Draw a left-aligned text from the current cursor position. The text can be multi-line and even multi-page. When
     * crossing page boundaries (<code>contentEndY</code>) a new page is automatically created. Newline characters force
     * explicit lineshift. Normal word-wrap is performed to keep the text between the current <code>leftMargin</code>
     * and <code>rightMargin</code>
     *
     * @param txt The text to be drawn
    ...
    public float drawText(String txt) {
        return drawText(txt, HexPDF2.LEFT);
    }

    /**
     * Draw a text from the specified position. The text can be multi-line and even multi-page. When crossing page
     * boundaries (<code>contentEndY</code>) a new page is automatically created. Newline characters force explicit
     * lineshift. Normal word-wrap is performed to keep the text between the current <code>leftMargin</code> and
     * <code>rightMargin</code>
     *
    ...
    /**
     * Draw an image starting at current cursor location. If no flags are given, the top-left corner of the image is
     * positioned at the current cursor position. If one of the flags
     * <code>HexPDF.LEFT | HexPDF.CENTER | HexPDF.RIGHT</code> is set, the image is adjusted between
     * <code>leftMargin, rightMargin</code> accordingly. The cursor location is kept unchanged unless the flag
     * <code>HexPDF.NEWLINE</code> is set. This is useful for adding pictures as layers. If <code>HexPDF.NEWLINE</code>
     * is set, the cursor is positioned at <code>leftMargin</code> immediately below the image.
     *
    ...
    public void drawImage(BufferedImage image, int flags) {
        ...
        if ((flags & HexPDF2.NEWLINE) > 0) {
            setCursor(contentStartX, imgY - lineSep);
        }
    }

    /**
     * Draw an image at the given location. If no flags are given, the top-left corner of the image is positioned at the
     * specified <code>x, y</code> location. If one of the flags <code>HexPDF.LEFT | HexPDF.CENTER | HexPDF.RIGHT</code>
     * is set, the image is adjusted horizontally between <code>leftMargin, rightMargin</code> accordingly. The top of
     * the image will always be at the given <code>y</code> position. The cursor location is kept unchanged unless the
     * flag <code>HexPDF.NEWLINE</code> is set. This is useful for adding pictures as layers. If
     * <code>HexPDF.NEWLINE</code> is set, the cursor is positioned at <code>leftMargin</code> immediately below the
     * image.
     *
     * @param image The image to be added
    ...
    /**
     * Add a table to the document starting at current cursor location. The input table must be a two-dimensional array
     * of <code>Object</code>, all rows must have the same number of columns. If the table extends beyond the page
     * boundary <code>contentEndY</code> a new page is automatically created and the table continues. Normal word-wrap
     * is performed within each cell if the text is longer than the column´ designated width. A table element (Object)
     * can be either a String or a BufferedImage
     *
     * @param table the table data. Objects should be String or BufferedImage
     * @param column_width array of column widths
     * @param column_flag array of flags for text alignment within columns, one of
     *            <code>HexPDF.LEFT | HexPDF.CENTER | HexPDF.RIGHT</code>
     * @param table_align flag for alignment of the table itself
     * @return the height of the table - if multipage then return the height on the last page.
     */
    public float drawTable(Object[][] table, float[] column_width, int[] column_flag, int table_align) {
        ...
        float free_space = contentWidth - table_width;
    ...
    /**
     * Set font size in points. Normally you should not use this directly. Instead, do setNormalFontSize() followed by
     * normalStyle()
     *
    ...
    /**
     * Set height of top margin. Height of writable area (contentHeight) is adjusted accordingly.
     *
    ...
    /**
     * Set height of bottom margin. Height of writable area (contentHeigth) is adjusted accordingly.
     *
    ...
    /**
     * Set width of left margin. Width of writable area (contentWidth) is adjusted accordingly.
     *
    ...
    /**
     * Set width of right margin. Width of writable area (contentWidth) is adjusted accordingly.
     *
    ...
    /**
     * Returns the underlying PDPageContentStream from pdfBox. Note that the PDPageContentStream will change whenever a
     * new page is created, it is hence important to call getPDPageContentStream() before any direct operation on the
     * content stream.
     *
    ...
    /**
     * Set text color. This will be reset if you change style.
     * 
    ...
    /**
     * Set page orientation. Takes effect only next tine a newpage() is explicitly or implicitly called.
     * 
    ...
    /**
     * Set page size to be used. Takes effect on the next explicit or automatic newPage() Default is
     * <code>PDPage.PAGE_SIZE_A4</code>
     * 
